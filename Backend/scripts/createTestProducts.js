  ...
  try {
    ...
    for (let i = 0; i < shipments.length && i < 6; i++) {
      ...
      try {
        ...
        console.log(`✅ Created product "${product.name}" for shipment ${shipment.tracking_number} (Status: ${shipment.status})`);
      } catch (error) {
        console.error(`❌ Failed to create product for shipment ${shipment.tracking_number}:`, error.message);
      }
    }
    ...
    products.forEach(product => {
      ...
      console.log(`Product: ${product.name} - Shipment: ${product.shipment_tracking_number} - Status: ${shipmentStatus}`);
    });
