...
async function fixShipmentStatusEnum() {
  try {
    ...
    const statusColumnMatch = createTableStatement.match(/status\s+enum\([^)]+\)/i);
    ...
    const updatedStatusColumnMatch = updatedCreateTableStatement.match(/status\s+enum\([^)]+\)/i);
    ...
    console.log(`✅ Fixed ${updateResult.affectedRows} shipments with empty status values`);
