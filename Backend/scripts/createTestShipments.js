  ...
  try {
    ...
    for (const shipmentData of testShipments) {
      try {
        ...
        console.log(
          `✅ Created shipment ${shipment.tracking_number} with status: ${shipment.status}`
        );
      } catch (error) {
        console.error(
          `❌ Failed to create shipment ${shipmentData.tracking_number}:`,
          error.message
        );
      }
    }
    ...
    shipments.forEach((shipment) => {
      console.log(
        `Shipment: ${shipment.tracking_number} - Status: ${shipment.status}`
      );
    });
