...
Shipment.hasMany(Notification, { foreignKey: 'shipment_id', as: 'notifications' });
  ...
  try {
    ...
    try {
      ...
      if (missingColumns.length > 0) {
        console.warn(`⚠️  Missing profile columns: ${missingColumns.join(', ')}`);
    ...
    if (error.name === 'SequelizeConnectionRefusedError' || error.name === 'SequelizeConnectionError') {
      ...
      console.warn('⚠️  Server will continue running - database sync can be retried later');
    }
